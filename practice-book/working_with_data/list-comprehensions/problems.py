...
# Problem 32
# Write a function `mutate` to compute all words
# generated by a single mutation on a given word.
# A mutation is defined as inserting a character,
# deleting a character, replacing a character, or
# swapping 2 consecutive characters in a string.
# Only consider letters from `a` to `z`.

def mutate(word):
    print 'Finish me'

# Problem 33
# Write a function `nearly_equal` to
# test whether two strings are nearly equal.
# Two strings `a` and `b` are nearly equal
# when `a` can be generated by a single mutation
# on `b`.
